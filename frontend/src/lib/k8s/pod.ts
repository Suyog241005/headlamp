  ...
  getLogs(...args: Parameters<oldGetLogs | newGetLogs>): () => void {
    ...
    function onResults(item: string) {
      ...
      if (isFirstResultOfStream) {
        ...
        if (lastTimestamp && linesList[0] === lastProcessedLine) {
          linesList.shift();
        }
      }
      ...
      const processedChunk = processLogLines(linesList);

      logs.push(processedChunk);
    ...
    function connect() {
      ...
      // Always request timestamps from the API so we can extract the last timestamp for reconnection
      let currentUrl = `/api/v1/namespaces/${self.getNamespace()}/pods/${self.getName()}/log?container=${container}&previous=${showPrevious}&timestamps=true&follow=${follow}`;
      if (lastTimestamp) {
      ...
      const { cancel } = stream(
        currentUrl,
        (item: string) => {
          if (isFirstResultOfStream && lastTimestamp) {
